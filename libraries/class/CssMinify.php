...
class CssMinify
{
    ...
    private $access = array(
        'server' => ROOT . 'assets/',
        'public_asset' => ROOT . 'public/assets',
        'asset' => ASSET . 'assets/',
        'folder' => 'caches/'
    );
    private $cacheName = '';
    ...
    public function vite($src){
        ...
        if(isset($manifest[$src]['css'])){
            echo '<link rel="stylesheet" href="public/'.$manifest[$src]['css'][0].'">';
        }
        return '<script type="module" src="public/'.$manifest[$src]['file'].'"></script>';
    }
}
